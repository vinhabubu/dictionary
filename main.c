...
void set_textView_text(char * text)  // dua tu vao bo nho dem
{
	...
	gtk_text_view_set_buffer(GTK_TEXT_VIEW(textView), buffer); // đưa ra ngoài màn hình
	
}

static char code[128] = { 0 };
const char* soundex(const char *s) // chuyen sang ma codex tham khao:https://ourcodeworld.com/articles/read/249/implementation-of-the-soundex-algorithm-function-in-different-programming-languages
{
	static char out[5];
	int c, prev, i;
...
//https://github.com/hajaulee/c-dictionary/blob/master/dict.c tham khảo ở đây phần soundex 
...
gboolean search_suggest(GtkWidget * entry, GdkEvent * event, gpointer No_need) //(CHECK) gioi han ky tu, chi nhan alphabelt va tab
{
...
int btfind(char * word) // Hàm tìm nghĩa của từ 
{
	char mean[100000];
	int size;
...
static void search(GtkWidget *w, gpointer data) // ham chuc nang search
{
	...
	a = gtk_entry_get_text(GTK_ENTRY(entry1));// Nhap vao 1 tu vao o "TRA"
	g_print("%s\n",a);
	char word[50];
	BTint x;
...
static void edit( GtkWidget *w, gpointer data ) // Cập nhật nghĩa của từ 
{
	GtkWidget *entry1= ((GtkWidget**)data)[0];
	GtkWidget *window1=((GtkWidget**)data)[1];
	...
	{
		Show_message(window1, GTK_MESSAGE_INFO, "Cách dùng:", "Chỉ có thể sửa từ đã có sẵn ."); // show_message o tren
		return;
	}
	...
	else if (bfndky(book, word, &x ) != 0)// bfndky là hàm tìm key .
		Show_message(GTK_WINDOW(window1), GTK_MESSAGE_ERROR, "Xảy ra lỗi!", "Từ vừa nhập không có trong từ điển.");
	else
	{
		if( btupd(book, word, mean, strlen(mean) + 1)==1)// câp nhật data từ 1 key có sẵn 
...
static void add(GtkWidget *w, gpointer data) // hàm thêm từ 
{

	GtkTextBuffer *buffer1,*buffer2;
	GtkTextIter start, end,iter;
	char mean[10000], word[50];
	buffer1 = gtk_text_view_get_buffer(GTK_TEXT_VIEW(GTK_TEXT_VIEW(view1))); // đưa input vào bộ nhớ đệm 
...
static void del(GtkWidget *w, gpointer data) // hàm xóa 
{
	GtkWidget *entry1= ((GtkWidget**)data)[0];
	GtkWidget *window1=((GtkWidget**)data)[1];
	...
	void tra_tu(GtkWidget widget, gpointer window)
	{
		...
		fixed = gtk_fixed_new();// tạo nền 
		gtk_container_add(GTK_CONTAINER(window1), fixed);

		table = gtk_table_new(2,4,FALSE);// tạo bảng 
		gtk_container_add(GTK_CONTAINER(fixed), table);

		label = gtk_label_new("Nhập:");// tạo nhãn 
		...
		gtk_widget_set_size_request(textView, 20, 300);
		...
		g_signal_connect(G_OBJECT(entry_search), "activate", G_CALLBACK(search), data); //ấn enter :activated 
	...
	void about(GtkWidget widget, gpointer window)
	{
		...
		gtk_about_dialog_set_comments(GTK_ABOUT_DIALOG(about_dialog), "Trần Huy Tú\n Lưu Văn Đông\nHoàng Gia Phong\nTrần Bảo Ngọc\n ");     
		gtk_about_dialog_set_copyright(GTK_ABOUT_DIALOG(about_dialog), "Thank you!!<3");
	...
	{
		btinit();	// Khởi tạo B-tree.
		book= btopn("words.dat",0,1);	//Đọc dữ liệu từ file từ điển
    ...
    //tao cua so
		window = gtk_window_new(GTK_WINDOW_TOPLEVEL);// tạo ra 1 cửa sổ rỗng 
		gtk_window_set_title(GTK_WINDOW(window), "Dictionary");// đặt tên cho cửa sổ
		gtk_window_set_default_size(GTK_WINDOW(window), 640, 480); //cài kích thước theo tọa độ yox
		gtk_window_set_position(GTK_WINDOW(window), GTK_WIN_POS_CENTER);// tạo của sổ giữa màn hình 

    //tao nen
		fixed = gtk_fixed_new();// tạo nền
		gtk_container_add(GTK_CONTAINER(window), fixed); // đưa nền vào của sổ

    //anh nen
		image = gtk_image_new_from_file("anh1.jpg");
		gtk_container_add(GTK_CONTAINER(fixed), image);

		btn1 = gtk_button_new_with_label("Tra từ");// tạo 1 button
		gtk_fixed_put(GTK_FIXED(fixed), btn1, 500, 20);// vị trí của nút theo xoy 
		gtk_widget_set_size_request(btn1, 240,100); // kích cỡ của button width,height 
		gtk_widget_set_tooltip_text(btn1, "Search a word");
		g_signal_connect(G_OBJECT(btn1), "clicked", G_CALLBACK(tra_tu), NULL);// khi click -> chạy vào hàm tra từ 

		btn2 = gtk_button_new_with_label("Thêm từ");
		gtk_fixed_put(GTK_FIXED(fixed), btn2, 500, 140);
		gtk_widget_set_size_request(btn2, 240, 100);
		...
		gtk_fixed_put(GTK_FIXED(fixed), btn3,500, 260);
		gtk_widget_set_size_request(btn3, 240, 100);
		...
		gtk_fixed_put(GTK_FIXED(fixed), btn4,500, 380);
		gtk_widget_set_size_request(btn4, 240, 100);
		g_signal_connect(G_OBJECT(btn4), "clicked", G_CALLBACK(about), NULL);

		btn5 = gtk_button_new_with_label("Thoát");
		gtk_fixed_put(GTK_FIXED(fixed), btn5,500, 500);
		gtk_widget_set_size_request(btn5, 240,100);
		gtk_widget_set_tooltip_text(btn5, "Exit");
		g_signal_connect(G_OBJECT(btn5), "clicked", G_CALLBACK(close_window), window);// hàm thoát 

		g_signal_connect(G_OBJECT(window), "destroy", G_CALLBACK(gtk_main_quit), NULL);// thoát khỏi chương trình 
